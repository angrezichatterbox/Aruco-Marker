...
def generate_marker(
    marker_id, size=200, output_path="marker.png", dict_type=aruco.DICT_6X6_250
):
    """
...
def draw_cube(
    image, rvec, tvec, camera_matrix, dist_coeffs, marker_length, z_offset=0.015
):
    """
...
def main():
    ...
    parser_gen.add_argument(
        ...
        type=str,
        default="marker.png",
        help="Output file path (default: marker.png)",
    )

    # Subparser for detecting from a webcam
    parser_det_webcam = subparsers.add_parser(
        "detect-webcam", help="Detect ArUco markers using the webcam"
    )
    parser_det_webcam.add_argument(
        "--camera", type=int, default=0, help="Camera index (default: 0)"
    )
    parser_det_webcam.add_argument(
        "--ar", action="store_true", help="Overlay a 3D cube (Augmented Reality)"
    )
